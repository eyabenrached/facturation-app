...
def _company_block(logo_path: str, styles):
    ...
    address = _env("INVOICE_COMPANY_ADDRESS", "Tunis - Tunisie")
    phone = _env("INVOICE_COMPANY_PHONE", "+216 00 000 000")
    email = _env("INVOICE_COMPANY_EMAIL", "contact@eurafrtours.tn")
    fiscal = _env("INVOICE_COMPANY_FISCAL_ID", "À renseigner")
    ice = _env("INVOICE_COMPANY_ICE", "À renseigner")
    ...
    info = [
        ...
        Paragraph(f"<b>ICE :</b> {_safe(ice)}", styles["company_info"]),
    ]
...
def _invoice_meta(facture, styles):
    ...
    echeance = date_facture + timedelta(days=echeance_days)
    mode = _env("INVOICE_PAYMENT_METHOD", "Virement")
    ...
    rows = [
        ...
        [Paragraph("Échéance", styles["meta_label"]), Paragraph(f":  {_fmt_date(echeance)}", styles["meta_value"])],
        [Paragraph("Mode de paiement", styles["meta_label"]), Paragraph(f":  {_safe(mode)}", styles["meta_value"])],
    ]
